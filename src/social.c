...
#include "db.h"
#include "const.h"
#define SOCIAL_FILE "../area/social.txt" /* or whatever fits you */
/*stuff to set up key*/
#if defined(KEY)
#undef KEY
#endif
...
void load_social_table()
{
	...
	{	social_table[i].name		= str_dup("");
		social_table[i].char_no_arg	= str_dup("");
		social_table[i].others_no_arg	= str_dup("");
		social_table[i].others_found	= str_dup("");
		social_table[i].others_auto	= str_dup("");
		social_table[i].vict_found	= str_dup("");
		social_table[i].char_auto	= str_dup("");
		social_table[i].chan_emote	= str_dup("");
		social_table[i].char_found	= str_dup("");
...
int social_lookup (const char *name)
{
    ...
    for (i = 0; social_table[i].name != NULL; i++)
    {
        if (LOWER (name[0]) == LOWER (social_table[i].name[0])
            && !str_prefix (name, social_table[i].name))
            return i;
    }
...
void do_sedit (CHAR_DATA *ch, char *argument)
{
	...
	else if (!str_cmp(cmd, "show")) /* Show a certain social */
	{
		sprintf (buf, "Social: %s\n\r"
		              "(chanmt) Used in chan social-emote:\n\r"
		              "%s\n\r\n\r"
